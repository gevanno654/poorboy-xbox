...
const LoginForm: React.FC = () => {
  ...
  // State untuk waktu update terakhir
        ...
        (querySnapshot) => {
          if (!querySnapshot.empty) {
            ...
            if (accountData.lastUpdate) {
              const updateDate = accountData.lastUpdate;
              const formattedDate = updateDate.toLocaleDateString('id-ID', {
                day: 'numeric',
                month: 'long',
                year: 'numeric'
              });
              const formattedTime = updateDate.toLocaleTimeString('id-ID', {
                hour: '2-digit',
                minute: '2-digit'
              });
              setLastUpdateTime(`Update terakhir: ${formattedDate} pukul ${formattedTime} WIB`);
            }
          } else {
            setAccountInfo(null);
            setLastUpdateTime('');
      ...
      <div className="max-w-xl mx-auto px-6 pt-6">
        ...
        <div className="bg-white/10 backdrop-blur-md border border-white/30 rounded-3xl p-6 mb-4">
          ...
          <div className="mt-4 mb-2">
            ...
            {/* PEMBERITAHUAN UPDATE TERAKHIR */}
            {lastUpdateTime && (
              <div className="mt-4 p-3 bg-yellow-500/20 border border-yellow-400/50 rounded-2xl">
                <div className="flex items-center justify-center gap-2">
                  <svg className="w-5 h-5 text-yellow-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  <span className="text-yellow-300 text-sm font-medium text-center">
                    {lastUpdateTime}
                  </span>
                </div>
              </div>
